...
# inference\images\
# inference\images\vids\Jur_1_demo.mp4
# inference\images\vids20s\Jur1_20s
# inference\images\vidsLong\Jur2_70s
# inference\images\vidsLonger\Jur2_23_06_2022.mp4
# 
# python detect_or_byteTrack_create_dataset.py --no-trace --view-img --source inference\images\Jur_1_demo.mp4 --track --classes 0 1 2 3 5 6 7 16 --show-track
# python detect_or_byteTrack_create_dataset.py --no-trace --view-img --source inference\images\Jur_2_demo.mp4 --track --classes 0 --show-track          vidsLonger\Jur2_23_06_2022.mp4
# python detect_or_byteTrack_create_dataset.py --no-trace --track --classes 0 --create-dataset --source inference\images\Jur_1_demo.mp4   
...
def detect(save_img=False):
            ...
            if len(det):
                ...
                if opt.track:
                    ...
                    for t in online_targets:
                        ...
                        tid = t[4]
                        tid_str = str(int(tid))
                        center_of_bbox_str = (f'{tlwh[2]:.1f}', f'{tlwh[3]:.1f}')
                        center_of_bbox_int = (int(tlwh[2]), int(tlwh[3]))
                        
                        if opt.create_dataset:
                            frame_positions = np.vstack((frame_positions, np.array([10.0 + frame_number, tid + 1.0, center_of_bbox_str[0], center_of_bbox_str[1]])))

                        if opt.show_track:
                            if tid_str in tracks_dict:
                                tracks_dict[tid_str].append(center_of_bbox_int)
                            else:
                                tracks_dict[tid_str] = [center_of_bbox_int]
    ...
    if final_pos.shape[0] > 0:
        with open('..\..\datasets\DatasetsJur\ByteTrack\SGAN-%s\pos_%s.txt' % (fps_label, p.name), 'a') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', default='runs/detect_or_byteTrack_create_dataset', help='save results to project/name')
    ...
    parser.add_argument('--create-dataset', action='store_true', help='create txt dataset')
    #######################################################
